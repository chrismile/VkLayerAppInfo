...
A Vulkan layer for logging the VkApplicationInfo struct values of apps.
...
export VK_LOADER_LAYERS_ENABLE=*appinfo
```
...
```shell
export VK_ADD_LAYER_PATH=$(pwd)/build
export VK_INSTANCE_LAYERS=VK_LAYER_CHRISMILE_appinfo
```

The environment variable `VK_APPINFO_OUTPUT_PATH` can be used to specify where `VkApplicationInfo` struct values should
be logged.

```shell
export VK_APPINFO_OUTPUT_PATH=<...>.txt
```
...
set VK_LOADER_LAYERS_ENABLE=*appinfo
set VK_APPINFO_OUTPUT_PATH=<...>.txt
```
...
$env:VK_LOADER_LAYERS_ENABLE = "*appinfo"
$env:VK_APPINFO_OUTPUT_PATH = <...>.txt
```
